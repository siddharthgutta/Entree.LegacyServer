...
describe('Message', () => {
  ...
  describe('#addTokenOrCreate()', () => {
    it('should add a token to a socket token correctly', done => {
      SocketToken.addTokenOrCreate(restaurantId, token).then(result => {
  ...
  describe('#removeToken()', () => {
    it('should remove a token from a socket token correctly', done => {
      SocketToken.addTokenOrCreate(restaurantId, token).then(() => {
        SocketToken.removeToken(restaurantId, token).then(result => {
          assert.equal(result.restaurantId, restaurantId);
  ...
  describe('#isValidToken', () => {
    it('should determine if a token is valid', done => {
      SocketToken.addTokenOrCreate(restaurantId, token).then(() => {
        SocketToken.isValidToken(restaurantId, token).then(result => {
  ...
  describe('#findOne', () => {
    it('should find the correct SocketToken given a restaurantId', done => {
      SocketToken.addTokenOrCreate(restaurantId, token).then(() => {
        SocketToken.addTokenOrCreate(restaurantId + 1, token).then(() => {
          SocketToken.findOne(restaurantId).then(result => {
            assert.equal(result.restaurantId, restaurantId);
            assert.equal(result.numTokens, 1);
