...
import {testSMS, sendSMS, sendTestSMS, broadcast} from "../api/sms.es6";
  ...
  */
  ...
  _.map(to_test_nums, test => {
    it(test.message, done => {
      sendTestSMS(test.to, `TEST SMS ${moment().format("h:mm A")}`)
          .then((response)=> {
            console.tag(TEST).log(response);
...
describe('Twilio Send: From', () => {
  _.map(from_test_nums, test => {
    it(test.message, done => {
        testSMS.changeFromNumber(test.num);
        sendTestSMS(to_test_nums.TO_VALID_NUMBER.num, `TEST SMS ${moment().format("h:mm A")}`)
        .then((response) => {
            console.tag(TEST).log(response);
            expect(test.error).to.be(false);
            done();
